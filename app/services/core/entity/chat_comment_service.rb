module Core
  module Entity
    class ChatCommentService
      ...
      def reading_comment!
        if @comments.where(entity_read: false, entity: false).present?
          new_comments = @comments.where(entity: false)
          new_comments.update_all(entity_read: true, entity_read_datetime: DateTime.now)
      ...
      def entity_notification!
        if @chat.chat_comments.count == 1
          entity_start_notification!
        else
