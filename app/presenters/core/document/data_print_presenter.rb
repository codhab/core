      ...
      def write_document(template)
        ...
        self.allotment.data_prints.where(complete_address: self.complete_address).where.not(id: self.id).each do |second|
          ...
          if second.spouse_cpf.present?
            ...
            template = second.wedding_regime.present? && second.wedding_date.present? ? template.gsub('doc.second_regime_casamento', "casados em #{second.wedding_regime} em #{second.wedding_date.strftime('%d/%m/%Y') rescue nil}") : template.gsub('doc.second_regime_casamento',"")
        ...
        if self.spouse_cpf.present?
          ...
          template = self.wedding_regime.present? && self.wedding_date.present? ? template.gsub('doc.regime_casamento', "casados em #{self.wedding_regime} em #{self.wedding_date.strftime('%d/%m/%Y') rescue nil}") : template.gsub('doc.regime_casamento',"")
