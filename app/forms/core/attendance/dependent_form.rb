    ...
    class DependentForm < Core::Candidate::DependentMirror
      ...
      def kinship_spouse
        if self.kinship_id == 6
          if self.cadastre_mirror.dependent_mirrors.where(kinship_id: 6).where.not(id: self.id).present?
            errors.add(:kinship_id, "Já existe um conjugê cadastrado")
          end
        end

        old_dependent = self.cadastre_mirror.dependent_mirrors.find(self.id)
